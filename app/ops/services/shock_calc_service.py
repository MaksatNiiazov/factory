...
def calculate_shock_block(data: dict, user):
    ...
    def get_parameters(self, available_options: Optional[Dict[str, Any]] = None) -> Tuple[Dict[str, Any], List[str]]:
        ...
        Основа — твоя рабочая версия, дополнено проставлением D из подобранного shock_item.
        """
        ...
        variant_data = available_options.get('suitable_variant') or {}
        selected_variant = None
        if variant_data:
            selected_variant = Variant.objects.filter(id=variant_data.get('id')).first()

        if not selected_variant:
            return parameters, locked_parameters

        detail_attrs = list(selected_variant.detail_type.attributes.all())
        variant_attrs = list(selected_variant.attributes.all())

        attrs: Dict[str, Attribute] = {a.name: a for a in detail_attrs}
        attrs.update({a.name: a for a in variant_attrs})

        shock_result = available_options.get('shock_result', {}) or {}

        # --- добавлено: найдём shock_item, чтобы достать из него D ---
        def _num(x):
            if x is None:
                return None
            try:
        ...
        # приоритет: fn из результата -> check_load -> разбор из маркировки -> self.get_load()
        check_load = _num(shock_result.get('fn')) or _num(shock_result.get('check_load'))
        if check_load is None:
            mk = (shock_result.get('marking') or '').split()
            if mk:
                part = mk[-1].split('.')[0]
                if part.isdigit():
                    check_load = _num(part)
        if check_load is None:
            check_load = self.get_load()

        shock_item = None
        try:
            if check_load is not None:
                shock_item = self.get_shock_item(selected_variant, check_load)
        except Exception:
            shock_item = None
        # --- конец добавленного блока ---

        usage_map = {
            AttributeUsageChoices.LOAD: lambda: self.get_load(),
            AttributeUsageChoices.RATED_STROKE: lambda: shock_result.get('stroke'),
        }

        # D берём из shock_item.parameters['D'], если он есть
        name_map = {
            'L': lambda: shock_result.get('l1'),
            'L2': lambda: shock_result.get('l2'),
            'L2_min': lambda: shock_result.get('l2_min'),
            'L2_max': lambda: shock_result.get('l2_max'),
            'L3_min': lambda: shock_result.get('l3_min'),
            'L3_max': lambda: shock_result.get('l3_max'),
            'L4': lambda: shock_result.get('l4'),
            'L_final': lambda: shock_result.get('l_final'),
            'extender': lambda: shock_result.get('extender'),
            'd': lambda: shock_result.get('mounting_length'),
            'type': lambda: shock_result.get('type'),
            'l_cold': lambda: shock_result.get('l_cold'),
            'D': (lambda: _num(shock_item.parameters.get('D')) if (shock_item and shock_item.parameters) else None),
        }

        for attr in attrs.values():
            value = None
            if attr.usage in usage_map:
                value = usage_map[attr.usage]()
            elif attr.name in name_map:
                value = name_map[attr.name]()

            if value is not None:
                parameters[attr.name] = value
                locked_parameters.append(attr.name)
